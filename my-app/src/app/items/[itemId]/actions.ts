...
export async function createBidAction(itemId: number) {
  ...
  // If currentBid is 0 (first bid), base the increment on the startingPrice.
  // Otherwise, base it on the currentBid.
  const baseBid = item.currentBid === 0 ? item.startingPrice : item.currentBid;
  const latestBidValue = baseBid + item.bidInterval;
  // --- END: MODIFIED LOGIC ---
  ...
  await database
    .update(items)
    .set({
      currentBid: latestBidValue, // Update the item's current bid
    })
    .where(eq(items.id, itemId));
